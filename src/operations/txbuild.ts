...
function getTransactionSigner(
  input: string | TransactionSigner
): TransactionSigner {
 ...
 * @private
 */
function estimateNamespaceReady(
  namespaceID: string,
...
function estimateTokenTransfer(
  ...
  return network.getFeeRate().then(feeRate => {
    ...
    const txFee =
      feeRate * estimateTXBytes(tokenTransferTX, senderUtxos, additionalOutputs)
    return txFee + outputsValue
  })
}
...
function makePreorder(
  ...
  return paymentKey.getAddress().then(preorderAddress => {
    const preorderPromise = Promise.all([
    ...
    return Promise.all([
      ...
      .then(([utxos, feeRate, preorderSkeleton]) => {
        ...
        const signingTxB = fundTransaction(
          txB,
          preorderAddress,
          utxos,
          feeRate,
...
function makeRevoke(
  ...
  return Promise.all([ownerKey.getAddress(), paymentKey.getAddress()])
    .then(([ownerAddress, paymentAddress]) => {
      const revokeTX = makeRevokeSkeleton(fullyQualifiedName)
      const txPromise = TransactionBuilder.fromTransaction(
        revokeTX,
        network.layer1
      )
      ...
      return Promise.all([
        ...
        if (ownerOutputAddr !== ownerAddress) {
          return Promise.reject(
            new Error(
              `Original owner ${ownerAddress} should have an output at ` +
...
function makeNamespaceReady(
  ...
  return revealKey
    .getAddress()
    .then(revealAddress =>
      Promise.all([network.getUTXOs(revealAddress), network.getFeeRate()]).then(
        ([utxos, feeRate]) => {
          const txB = TransactionBuilder.fromTransaction(
            namespaceReadyTX,
            network.layer1
...
function makeAnnounce(
  ...
  return senderKey
    .getAddress()
    .then(senderAddress =>
      Promise.all([network.getUTXOs(senderAddress), network.getFeeRate()]).then(
        ([utxos, feeRate]) => {
          const txB = TransactionBuilder.fromTransaction(
            announceTX,
            network.layer1
...
function makeBitcoinSpend(
  ...
  if (amount <= 0) {
    return Promise.reject(
      new InvalidParameterError('amount', 'amount must be greater than zero')
    )
  }
