...
export function hexStringToECPair(skHex: string): ECPair.ECPairInterface {
  ...
  if (skHex.length === 66) {
    ...
    return ECPair.fromPrivateKey(
      Buffer.from(skHex.slice(0, 64), 'hex'),
      ecPairOptions
    )
...
export function ecPairToAddress(keyPair: ECPair.ECPairInterface) {
  return address.toBase58Check(
    crypto.hash160(keyPair.publicKey),
...
export function isSameOriginAbsoluteUrl(uri1: string, uri2: string) {
  ...
  const port1 =
    parseInt(parsedUri1.port || '0', 10) | 0 ||
    (parsedUri1.protocol === 'https:' ? 443 : 80)
  const port2 =
    parseInt(parsedUri2.port || '0', 10) | 0 ||
...
export function getGlobalObject<K extends keyof Window>(
  ...
  try {
    ...
    Logger.error(
      `Error getting object '${name}' from global scope '${globalScope}': ${error}`
    )
  }
  if (throwIfUnavailable) {
    const errMsg = getAPIUsageErrorMessage(
      globalScope,
      name.toString(),
      usageDesc
    )
