 ...
 * @private
 */
async function decryptLegacy(dataBuffer: Buffer, password: string) {
 ...
 * @private
 */
export async function decryptMnemonic(data: (string | Buffer), password: string): Promise<string> {
  ...
  try {
    ...
    const data = await decryptLegacy(dataBuffer, password)
