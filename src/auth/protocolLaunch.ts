...
export function launchCustomProtocol(
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (storageKey && storageKey.startsWith(echoReplyKeyPrefix)) {
          ...
          if (
            ...
            Date.now() - parseInt(storageValue, 10) > 3600000
          ) {
  ...
  const hiddenCssStyle =
    'all: initial; position: fixed; top: 0; height: 0; width: 0; opacity: 0;'
  ...
  setTimeout(
    () =>
      document.removeEventListener('visibilitychange', pageVisibilityChanged),
    500
  )
  ...
  addEventListener(
    ...
    function replyEventListener(event) {
      if (
        ...
        localStorage.getItem(echoReplyKey) === 'success'
      ) {
  ...
  const iframeStyle =
    'all: initial; display: none; position: fixed; top: 0; height: 0; width: 0; opacity: 0;'
