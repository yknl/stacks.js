...
import { BLOCKSTACK_HANDLER, nextMonth } from '../utils'
import { DEFAULT_BLOCKSTACK_HOST, DEFAULT_SCOPE } from './authConstants'
...
export function makeAuthRequest(transitPrivateKey: string,
  ...
  const appConfig = new AppConfig(scopes, appDomain, redirectURI, manifestURI)
  const userSession = new UserSession({ appConfig })
  const transitKey = (transitPrivateKey == null) 
    ? userSession.generateAndStoreTransitKey() : transitPrivateKey
  return userSession.makeAuthRequest(transitKey, expiresAt, extraParams)
}
...
export function redirectToSignInWithAuthRequest(authRequest: string,
  ...
  userSession.redirectToSignInWithAuthRequest(sessionAuthRequest)
}
