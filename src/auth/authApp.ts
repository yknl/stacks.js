...
export function isSignInPending() {
  try {
    ...
    Logger.error(
      `Error checking for protocol echo reply isSignInPending: ${error}`
    )
  }
...
export function redirectToSignInWithAuthRequest(
  ...
  const { navigator, location } = getGlobalObjects(['navigator', 'location'], {
    throwIfUnavailable: true,
...
export async function handlePendingSignIn(
  ...
  try {
    ...
    if (isProtocolEcho) {
      const msg =
        'handlePendingSignIn called while protocolEchoReply was detected, and ' +
      ...
      return new Promise<UserData>((_resolve, reject) => {
        setTimeout(() => {
          Logger.error(
            'Page should have redirected by now. handlePendingSignIn will now throw.'
          )
          reject(msg)
        }, 3000)
      })
    ...
    Logger.error(
      `Error checking for protocol echo reply handlePendingSignIn: ${error}`
    )
  }
  ...
  if (!nameLookupURL) {
    ...
    if (
      isLaterVersion(tokenPayload.version, '1.3.0') &&
      tokenPayload.blockstackAPIUrl !== null &&
      tokenPayload.blockstackAPIUrl !== undefined
    ) {
      // override globally
      Logger.info(
        `Overriding ${config.network.blockstackAPIUrl} ` +
  ...
  if (isLaterVersion(tokenPayload.version, '1.1.0')) {
    if (transitKey !== undefined && transitKey != null) {
      if (
        ...
        try {
          appPrivateKey = decryptPrivateKey(
            transitKey,
